        ...
        <nav class="navbar navbar-expand-lg blur border-radius-lg top-0 z-index-3 shadow position-absolute mt-4 py-2 start-0 end-0 mx-4">
          <div class="container-fluid ps-2 pe-0">
            <a class="navbar-brand font-weight-bolder ms-lg-0 ms-3 " href="#">
             Sistem Informasi Persediaan Barang - Toko Ragil Jaya
            </a>
          ...
          <!-- end alert -->
                <div class="card-body">
                  <form role="form" action="{{url('register')}}" method="POST">
                    ...
                    @enderror
                    <div class="input-group input-group-outline mb-3">
                        <input type="number" name="no_telp" class="form-control @error('no_telp') is-invalid @enderror" value="{{old('no_telp')}}" placeholder="Nomer Telp">
                    </div>
                    @error('password')
                    <div class="form-text text-danger">{{$message}}.</div>
                    @enderror
                    ...
                    <div class="input-group input-group-outline mb-3">
                        <select class="form-select p-2 @error('role') is-invalid @enderror" value="{{old('role')}}" name="role" id="role" >
                            @foreach ($data_role as $role)
                            <option value="{{$role['kd_role']}}" {{ old('role') == $role['kd_role'] ? 'selected' : '' }}>{{$role['nama_role']}}</option>
                            @endforeach
                          </select>
                          @error('role')
                        <div class="form-text text-danger">{{$message}}.</div>
